...
//!
//! Each behavior is a separate setting, and only `craftingPool` is on. It
//! filters what a crafting pull draws from: workshop crafting, the build hammer,
//! the Material Catalog and factory pulls see only the acting player's own
//! scope. `restrictOpen` and `quickStack` extend the same ownership model to
//! opening a container and to quick-stack deposits.
//!
